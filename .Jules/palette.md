...
## 2025-12-25 - Drag-and-Drop Accessibility Gaps
**Learning:** `makeDraggable` in `js/engine/input.js` handles touch/mouse drag but lacks native click/keyboard fallback. Games relying solely on it (like Math Game) become inaccessible to keyboard users and difficult for those with motor impairments.
**Action:** When using drag-and-drop patterns, always implement a parallel `onclick` / `onkeydown` handler on the draggable elements that programmatically triggers the drop logic.
