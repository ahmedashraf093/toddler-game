...
## 2026-02-12 - Missing CSS for Visual Feedback
**Learning:** JS logic for accessibility feedback (like `wiggle-error`) can silently fail if the corresponding CSS class is missing. The lack of visual feedback might go unnoticed during manual testing if audio feedback is present, but it degrades the experience for users who rely on visual cues.
**Action:** When implementing visual feedback in JS (adding classes), always verify that the CSS class is defined in the codebase.
